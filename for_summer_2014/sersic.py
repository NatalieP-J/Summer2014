...
class SersicModel:
	...
	def rho(self,r):
		return ((r/self.Re)**-self.p)*exp(-self.b*((r/self.Re)**(1./self.n)))
...
plt.figure()
plt.loglog(rtest[1:-1],model1.rho(rtest[1:-1])/max(model1.rho(rtest[1:-1])),'SpringGreen',linewidth = 5.,label = 'n = {0}'.format(1))
plt.loglog(rtest[1:-1],model2.rho(rtest[1:-1])/max(model2.rho(rtest[1:-1])),'Gold',linewidth = 5.,label = 'n = {0}'.format(2))
plt.loglog(rtest[1:-1],model3.rho(rtest[1:-1])/max(model3.rho(rtest[1:-1])),'DeepSkyBlue',linewidth = 5.,label = 'n  = {0}'.format(4))
#plt.axvline(1)
plt.ylim(1e-8,10)
plt.xlim(1e-3,1e2)
plt.xlabel(r'radius [$R_{eff}$]')
plt.ylabel(r'density [$\rho_{centre}$]')
plt.legend(loc = 'best')
