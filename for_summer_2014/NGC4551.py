...
MBHs = 10**MBHs
names = data[:,0]
rbs = array([float(i) for i in data[:,3]])
rbs = 10**rbs
...
########******************* CONSTRUCT MODEL *******************########
    ...
    model = NukerModel(name,alpha,beta,gamma,rb,rho0,MBH,generate)            
...
########******************* CONSTRUCTION FUNCTIONS *******************########
    ...
    def plotter(name,tab,rarray,r,inter,rstart,rchange,start,end,smallrexp,largerexp,conds,labels):
        """
    ...
    def makegood(func,r,size,grid,smallrexp,largerexp,verbose = False,conds = False,plotting=False,problem = True):
        ...
        print 'fraction reporting a message: {0}/{1} = {2}'.format(len(problems),len(tab),prob)
        statfile.write('\nfraction reporting a message: {0}/{1} = {2}'.format(len(problems),len(tab),prob))
        bigstatfile.write('\nfraction reporting a message: {0}/{1} = {2}'.format(len(problems),len(tab),prob))
        if problem == True:
            tab = array([i for j, i in enumerate(tab) if j not in problems])
            rarray = array([i for j, i in enumerate(rarray) if j not in problems])
        ...
        if float(len(nanstrack))/float(len(tab)) > 0.3:
            ...
            inter = interp1d(log10(rarray),log10(tab))#,kind = 'quadratic')
            ...
            if plotting != False:
               p = plotter(str(func)[10:15],tab,rarray,r,inter,rstart,rchange,start,end,smallrexp,largerexp,conds,plotting)
               if p > 0:
                   return 'Fail'
               else:
                   return inter
...
########******************* COMPUTE MENC *******************######## 
    ...
    statfile.write('\nStarting Menc')
    bigstatfile.write('\nStarting Menc')
    Mencgood = compute([],["Menc",Menc],funcMenc,rtest,[5,-5,40,-40,0.03],rgrid,[3-model.g,0],[[2,0,3-model.b,4*pi*model.rho(rchange)*(rchange**3)],['r','M'],False])
    if Mencgood == 'Fail':
        print 'Mencgood = ',Mencgood
        statfile.write('\nMencgood = {0}'.format(Mencgood))
        statfile.close()
        bigstatfile.write('\nMencgood = {0}'.format(Mencgood))
        continue
...
########******************* COMPUTE PSI *******************######## 
    ...
    psigood = compute([],["psi",psi],funcpsi,rtest,[5,-5,40,-40,0.03],rgrid,[-1,-1],[False,['r','$\psi$'],False])

    if psigood == 'Fail':
        print 'psigood = ',psigood
        statfile.write('\npsigood = {0}'.format(psigood))
        statfile.close()
        bigstatfile.write('\npsigood = {0}'.format(psigood))
        continue
########******************* APOCENTER RADIUS *******************######## 
...
########******************* COMPUTE Jc2 *******************######## 
    ...
    Jc2good = compute(prereqs,["Jc2",Jc2],funcJc2,rtest,[5,-5,12,-12,0.01],Egrid,[-1,-1],[False,['E','Jc2'],False])

    if Jc2good == 'Fail':
        print 'Jc2good = ',Jc2good
        statfile.write('\nJc2good = {0}'.format(Jc2good))
        statfile.close()
        bigstatfile.write('\nJc2good = {0}'.format(Jc2good))
        continue
